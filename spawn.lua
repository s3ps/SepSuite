...
local Create = function(Lib, Item, Props)
	local Inst = Lib["new"](Item)
...
local Chat = Get("Chat")
local RepFirst = Get("ReplicatedFirst")
local RepStorage = Get("ReplicatedStorage")
local CoreGui = Get("CoreGui")
local Workspace = Get("Workspace")
local Run = Get("Run")
local Http = Get("HttpService")
local Input = Get("UserInputService")
local Tween = Get("TweenService")
local Text = Get("TextService")
local Camera = Workspace.Camera
local VirtualUser = Get("VirtualUser")
local RGBToColor = function(Theme)
	return Color3.fromRGB(Theme.R, Theme.G, Theme.B)
end
local CurveUI = function(Insta)
...
-- Variables
local getreg = getreg or debug.getregistry or false
...
local getrawmetatable = getrawmetatable or getmetatable or debug.getmetatable
...
function Loop:While(Callback, Enabled, Delay)
	local Current = {}
	Current.Enabled = Enabled or false
	Current.Callback = Callback
	function Current:Toggle(Bool)
		Current.Enabled = Bool or not Current.Enabled
	end
	spawn(function()
		while wait(Delay) do
...
function Loop:Render(Callback, Enabled, Delay)
	local Current = {}
	Current.Enabled = Enabled or false
	function Current:Toggle(Bool)
		Current.Enabled = Bool or not Current.Enabled
...
function Loop:Stepped(Callback, Enabled, Delay)
	local Current = {}
	Current.Enabled = Enabled or false
	function Current:Toggle(Bool)
		Current.Enabled = Bool or not Current.Enabled
...
function Loop:Heartbeat(Callback, Enabled, Delay)
	local Current = {}
	Current.Enabled = Enabled or false
	function Current:Toggle(Bool)
		Current.Enabled = Bool or not Current.Enabled
...
-- Json Library
local Json = {
	Encode = function(...) return Http:JSONEncode(...) end,
	Decode = function(...) return Http:JSONDecode(...) end
}
...
local AccentR = Settings.File.Accent[1]
local AccentG = Settings.File.Accent[2]
local AccentB = Settings.File.Accent[3]
local RTAccent = (AccentR..","..AccentG..","..AccentB)

local Blur = Create(Instance, "BlurEffect", {
	Parent = Camera,
	Name = "Sephub",
	Size = 0
})
...
Tween:Create(Blur, TweenInfo.new(1, Enum.EasingStyle.Quad), {
	Size = 64
}):Play()

local ScreenGui = Create(Instance, "ScreenGui", {
	Parent = CoreGui,
	Name = "Sephub",
	ResetOnSpawn = false
})
...
local Container = Create(Instance, "Frame", {
	Parent = ScreenGui,
	BackgroundColor3 = Color3.fromRGB(unpack(Settings.File.Back)),
	Size = UDim2.new(0,400,0,0),
	Position = UDim2.new(.5,-200,.5,0),
	BorderSizePixel = 0,
	ClipsDescendants = true
})

Tween:Create(Container, TweenInfo.new(1, Enum.EasingStyle.Quad), {
...
local Title = Create(Instance, "TextLabel", {
	Parent = Container,
	RichText = true,
	TextColor3 = Color3.fromRGB(255,255,255),
	BackgroundTransparency = 1,
	Text = "SEP<font color='rgb("..RTAccent..")'>SUITE</font>",
...
Tween:Create(Title, TweenInfo.new(1, Enum.EasingStyle.Quad), {
	TextTransparency = 0
}):Play()
...
local Loading = Create(Instance, "Frame", {
	...
	BorderSizePixel = 0,
	BackgroundTransparency = 1,
	BackgroundColor3 = Color3.fromRGB(unpack(Settings.File.Accent)),
})
Tween:Create(Loading, TweenInfo.new(1, Enum.EasingStyle.Quad), {
